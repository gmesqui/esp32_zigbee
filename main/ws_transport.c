...
"function hex16(n){return n==null?'-':'0x'+Number(n).toString(16).padStart(4,'0').toUpperCase()}"
...
"function applyInventoryDevice(dev){if(!dev||!dev.device_id)return;let d=ensureDevice(dev.device_id);if(!d)return;d.name=dev.name||d.name;if(dev.nwk!=null)d.nwk=dev.nwk;if(dev.online!=null)d.online=!!dev.online;if(dev.state)d.state=dev.state;if(dev.manufacturer!=null)d.manufacturer=dev.manufacturer;if(dev.model!=null)d.model=dev.model;if(dev.power_source!=null)d.power_source=dev.power_source;if(dev.last_seen_s!=null)d.last_seen_s=dev.last_seen_s;if(dev.lqi!=null)d.lqi=dev.lqi;if(dev.rssi!=null)d.rssi=dev.rssi;d.is_sleepy=!!dev.is_sleepy;if(dev.reporting)d.reporting=dev.reporting;if(dev.stats)d.stats=dev.stats;if(dev.endpoints)d.endpoints=dev.endpoints;if(dev.capabilities)d.capabilities=dev.capabilities}"
...
static esp_err_t web_get_status_handler(httpd_req_t *req)
{
    ...
    for (int i = 0; i < MAX_DEVICES; i++) {
        ...
        web_json_stream_printf(&out,
                               ",\"nwk\":%u,\"online\":%s,\"is_sleepy\":%s,\"state\":",
                               dev_snapshot->nwk_addr,
